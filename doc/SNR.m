...
data1 = csvread("./EMBO_SCOPE_600kSps_const.csv", 1, 0);
%data = csvread("./EMBO_SCOPE_6kSps_6Hz.csv", 1, 0);
data2 = csvread("./EMBO_SCOPE_6kSps_const.csv", 1, 0);
fs1 = 600000;
fs2 = 6000;
...
trim = 30;
X_lp1 = lowpass(X1, 30000, fs1);
X_lp1 = X_lp1(trim:end-trim);
t_lp1 = t1(trim:end-trim);

X_lp2 = lowpass(X2, 300, fs2);
...
title("Měřené konstatní napětí baterie (f_s = 600 kHz)");
xlabel("Čas [ms]")
ylabel("Napětí [mV]")
grid on;
...
title("Měřené konstatní napětí baterie (f_s = 6 kHz)");
xlabel("Čas [ms]")
ylabel("Napětí [mV]")
grid on;
l=legend('původní signál','filtrovaný signál','Location','northeast');
...
subplot(2, 2, 3)
AllValue = X1;
fill([rms1-std1 rms1-std1 rms1+std1 rms1+std1], [0 1000 1000 0], [.9 .9 .9], 'linestyle', '--','LineWidth',2,'facealpha',0.7); hold on;
histogram(AllValue,'BinWidth',0.8,'facecolor', c('blue'));
title('Histogram měřených hodnot (f_s 600 kHz)');
...
subplot(2, 2, 4)
AllValue = X2;
fill([rms2-std2 rms2-std2 rms2+std2 rms2+std2], [0 1000 1000 0], [.9 .9 .9], 'linestyle', '--','LineWidth',2,'facealpha',0.7); hold on;
histogram(AllValue,'BinWidth',0.8,'facecolor', c('blue'));
title('Histogram měřených hodnot (f_s 6 kHz)');
...
lsb = -7.239e-007*t + 9.784;
lsb2 = -1.002e-007*t2 + 9.751;
...
l=legend(" STM32F103 ADC   Q_e = ± 2-3 mV", " ideální 12-bit ADC  Q_e = ± 0.4 mV", " nulová chyba (analogový signál)" , 'Location','best');
l.FontSize = 18;
grid on;
